...
template<class T> std::vector<T> sort_unique (std::vector<T> &v) { std::sort(all(v)), v.erase(std::unique(all(v)), v.end()); }
...
int main (int argc, const char *argv[]) {
    ...
    REP (i, n) pos[a[i]] = i;
    set<int> st;
    FOR (i, 1, k) st.insert(pos[i]);
    int mn = (*st.rbegin() - *st.begin());
    FOR (i, k + 1, n) {
        st.insert(pos[i]);
        st.erase(pos[i - k]);
        chmin(mn, (*st.rbegin() - *st.begin()));
    }
    print(mn);
    return 0;
}