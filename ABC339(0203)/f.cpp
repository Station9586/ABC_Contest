...
int main (int argc, const char *argv[]) {
    ...
    cin >> n;
    vi mod = {(ll)1e9 + 7, (ll)998244353, (ll)487631314};
    string s;
    vector<array<ll, 3>> num(n);
    for (int i = 0; i < n; i ++) {
        cin >> s;
        for (int j = 0; j < 3; j ++) {
            ll cal = 0;
    ...
    for (int i = 0; i < n; i ++) {
        for (int j = 0; j < n; j ++) {
            array<ll, 3> nw;
            for (int k = 0; k < 3; k ++) 
                nw[k] = num[i][k] * num[j][k] % mod[k];
            comb.emplace_back(nw);
        }
    }
    sort(all(comb));
    ll ans = 0;
    for (int i = 0; i < n; i ++) 
        ans += upper_bound(all(comb), num[i]) - lower_bound(all(comb), num[i]);
    cout << ans << "\n";
    return 0;
}