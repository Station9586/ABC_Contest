...
template<class T, class U> std::ostream& operator << (std::ostream& out, std::pair<T, U>& p) { out << p.first << " " << p.second; return out; }
template<class T> std::ostream& operator << (std::ostream& out, std::vector<T>& a) { for (T x: a) out << x << "\n"; return out; }
...
int main (int argc, const char *argv[]) {
    ...
    for (int i = 0; i < mxN; i ++) {
        if (cnt[i]) ans.emplace_back(i, cnt[i]);
    }
    cout << sz(ans) << "\n" << ans;
    return 0;
}